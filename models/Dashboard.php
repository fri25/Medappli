...
class Dashboard {
    ...
    public function getDerniersPatients($limit = 5) {
        $query = "SELECT p.id, p.nom, p.prenom, p.datenais, p.sexe, p.email, p.contact, r.derniere_visite 
                 FROM patient p 
                 INNER JOIN (
                     SELECT idpatient, MAX(dateheure) AS derniere_visite
                     FROM rendezvous
                     WHERE idmedecin = ?
                     GROUP BY idpatient
                 ) r ON p.id = r.idpatient
                 ORDER BY r.derniere_visite DESC
                 LIMIT ?";
        $stmt = $this->db->prepare($query);
