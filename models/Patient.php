...
class Patient extends User {
    ...
    public function register() {
        ...
        try {
            ...
            $writeLog("Erreur PDO lors de l'enregistrement : " . $e->getMessage());
            if (strpos($e->getMessage(), "Field 'id' doesn't have a default value") !== false) {
                $nextId = $this->getNextId();
                $query = "INSERT INTO " . $this->table_name . " 
                          (id, nom, prenom, datenais, sexe, email, contact, password, role, id_medecin,
                           verification_status) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

                $writeLog("Réessayer avec id explicite : " . $nextId);
                $stmt = $this->db->prepare($query);
                $stmt->bindParam(1, $nextId, PDO::PARAM_INT);
                $stmt->bindParam(2, $this->nom);
                $stmt->bindParam(3, $this->prenom);
                $stmt->bindParam(4, $this->datenais);
                $stmt->bindParam(5, $this->sexe);
                $stmt->bindParam(6, $this->email);
                $stmt->bindParam(7, $this->contact);
                $stmt->bindParam(8, $hashed_password);
                $stmt->bindParam(9, $this->role);
                $stmt->bindParam(10, $this->id_medecin);
                $stmt->bindParam(11, $this->verification_status);

                try {
                    if ($stmt->execute()) {
                        $writeLog("Compte enregistré avec succès avec id explicite");
                        return true;
                    }
                } catch (PDOException $e2) {
                    $writeLog("Erreur PDO lors de l'enregistrement avec id explicite : " . $e2->getMessage());
                }
            }
        ...
        $writeLog("Erreur lors de l'enregistrement : " . implode(", ", $errorInfo));
        return false;
    }
