...
class Config {
    ...
    public static function init() {
        try {
            ...
            $required_db_params = ['DB_HOST', 'DB_NAME', 'DB_USER', 'DB_PORT'];
            ...
            self::$config['database']['port'] = env('DB_PORT');
    ...
    public static function getDbConnection() {
        try {
            ...
            // Construire le DSN en incluant le port si défini
            $dsn = "mysql:host={$host};dbname={$dbname};charset={$charset}";
            if (!empty($port)) {
                $dsn .= ";port={$port}";
            }

            // Retirer l'option SSL CA si le fichier n'existe pas localement
            if (isset($options[PDO::MYSQL_ATTR_SSL_CA]) && !file_exists($options[PDO::MYSQL_ATTR_SSL_CA])) {
...
// Fonction raccourci pour générer une URL complète de l'application
function app_url($path = '') {
    $baseUrl = rtrim(env('APP_URL', 'http://localhost/medapp'), '/');
    if ($path === '' || $path === null) {
        return $baseUrl;
    }
...
// Fonction raccourci pour rediriger vers une URL de l'application
