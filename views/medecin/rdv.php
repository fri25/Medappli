                        ...
                        <div class="space-y-4">
                            <?php foreach ($rendezvous as $rdv): ?>
                                <div class="p-4 bg-[#F1F8E9] rounded-lg hover:bg-[#E8F5E9]">
                                    ...
                                    <form method="post" action="update_rdv_status.php" class="mt-4" id="form-<?= $rdv['id'] ?>">
                                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                        <input type="hidden" name="rdv_id" value="<?= $rdv['id'] ?>">
                                        <input type="hidden" name="update_status" value="1">
                                        
                                        <div class="flex items-center space-x-2">
                                            <select name="statut" data-old-value="<?= htmlspecialchars($rdv['statut']) ?>" class="p-2 border rounded">
...
function updateStatus(rdvId) {
    ...
    updateStatusBadge(statusSpan, newStatus);
    ...
    .then(data => {
        if (data.success) {
            ...
            updateStatusBadge(statusSpan, oldValue);
...
function updateStatusBadge(span, status) {
    const normalized = status.toLowerCase().trim();
    if (normalized === 'confirmé') {
        span.className = 'px-3 py-1 rounded-full text-sm status-badge bg-green-200 text-green-800';
    } else if (normalized === 'annulé') {
        span.className = 'px-3 py-1 rounded-full text-sm status-badge bg-red-200 text-red-800';
    } else {
        span.className = 'px-3 py-1 rounded-full text-sm status-badge bg-yellow-200 text-yellow-800';
    }
}
