...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $dateheure = date('Y-m-d H:i:s', $timestamp);
        ...
        $stmt = db()->prepare(
            "INSERT INTO rendezvous (idpatient, idmedecin, idspecialite, dateheure, statut)
            VALUES (?, ?, ?, ?, 'en attente')"
        );
        $stmt->execute([$idpatient, $idmedecin, $idspecialite, $dateheure]);

        if (!$stmt->rowCount()) {
            throw new Exception('Une erreur est survenue lors de la prise de rendez-vous.');
        }
