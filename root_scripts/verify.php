...
if (!isset($_GET['token']) || empty($_GET['token'])) {
    header('Location: ' . app_url('views/login.php') . '?error=' . urlencode('Token de vérification manquant'));
    exit;
}
...
if (!preg_match('/^[a-f0-9]{64}$/', $token)) {
    header('Location: ' . app_url('views/login.php') . '?error=' . urlencode('Format de token invalide'));
    exit;
}
...
try {
    ...
    if ($userVerified) {
        ...
        header('Location: ' . app_url('views/login.php') . '?success=' . urlencode('Votre compte a été vérifié avec succès. Vous pouvez maintenant vous connecter.'));
        exit;
    } else {
        // Si aucun compte n'a été mis à jour, le token est invalide ou expiré
        $db->rollBack();
        header('Location: ' . app_url('views/login.php') . '?error=' . urlencode('Token de vérification invalide ou expiré'));
        exit;
    }
    ...
    header('Location: ' . app_url('views/login.php') . '?error=' . urlencode('Une erreur est survenue lors de la vérification de votre compte'));
    exit;
}
?>
