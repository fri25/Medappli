...
if (file_exists($envFile)) {
    ...
    // Fournir une URL par défaut si elle n'est pas définie
    if (!isset($_ENV['APP_URL']) || $_ENV['APP_URL'] === '') {
        $_ENV['APP_URL'] = 'http://localhost/medapp';
    } else {
        $_ENV['APP_URL'] = rtrim($_ENV['APP_URL'], '/');
    }

    // Vérifier le secret de session en production
    if (isset($_ENV['APP_ENV']) && $_ENV['APP_ENV'] === 'production') {
        if (!isset($_ENV['SESSION_SECRET']) || $_ENV['SESSION_SECRET'] === '') {
            die('SESSION_SECRET est requis en production.');
        }
    }
} else {
